...
- [x] 2.1 在 `laokou-admin-client` 的字典 DTO/CO 或对应 command 校验中补齐必填、长度、状态值和 ID 校验。
- [x] 2.2 在 `dict` 领域服务和网关中补齐租户内字典类型唯一性校验。
- [x] 2.3 在 `dictItem` 领域服务和网关中补齐 `typeId` 归属校验和同类型字典项 `value` 唯一性校验。
- [x] 2.4 实现字典类型删除保护：存在未删除字典项时拒绝删除并返回明确业务错误。
- [x] 2.5 核对导入接口 `@RequestPart("files")`、导出接口 blob 响应和现有 OpenAPI 生成结果，修正不一致的契约。
- [x] 2.6 为字典类型和字典项补充 controller/app/domain/infrastructure 层关键测试。
...
- [x] 4.1 运行后端相关 Maven 测试，至少覆盖 `laokou-admin` 字典与字典项用例。
- [x] 4.2 运行前端类型检查、lint 或 build，确认新增页面可编译。
- [x] 4.3 手工验证权限矩阵：无权限隐藏按钮，缺少 scope 时后端拒绝。
- [x] 4.4 手工验证主要流程：分页查询、新增、修改、删除保护、导入、导出、失败提示。
- [x] 4.5 更新项目二开文档或数据库说明，记录数据字典入口、权限和迁移注意事项。
