...
public class GatewayReadPropertyCmdExe {
	...
	public Long execute(GatewayReadPropertyCmd cmd) {
		...
		gatewayCommandE.setDeviceKey(cmd.getDeviceKey());
	...
	private String toPayload(GatewayReadPropertyCmd cmd) {
		if (cmd.getIdentifiers() == null || cmd.getIdentifiers().isEmpty()) {
			return null;
		}
		return JacksonUtils.toJsonStr(cmd.getIdentifiers());
	}
