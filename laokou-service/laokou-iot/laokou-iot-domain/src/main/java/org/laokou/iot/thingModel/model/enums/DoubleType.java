...
public class DoubleType implements Serializable {
	...
	private final BigDecimal minVal = new BigDecimal("-1000000000.000000");
	...
	public ParamValidator.Validate checkValue() {
		...
		if (StringUtils.hasText(min)) {
			if (BigDecimalUtils.isScaleValid(min, 6)) {
				return ParamValidator.invalidate("最小值小数位不允许超过6位");
			}
			minValue = new BigDecimal(min).setScale(6, RoundingMode.HALF_UP).stripTrailingZeros();
		}
		if (StringUtils.hasText(max)) {
			if (BigDecimalUtils.isScaleValid(max, 6)) {
				return ParamValidator.invalidate("最大值小数位不允许超过6位");
			}
