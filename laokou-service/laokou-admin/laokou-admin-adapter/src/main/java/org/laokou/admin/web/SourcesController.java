...
public class SourcesController {
	...
	@PreAuthorize("hasAuthority('write') and hasAuthority('sys:source:modify')")
	@OperateLog(module = "数据源管理", operation = "修改数据源")
	@Operation(summary = "修改数据源", description = "修改数据源")
	@Cache(name = NameConstants.SOURCES, key = "#cmd.co.id", operateType = OperateType.DEL)
	public void modifySource(@RequestBody SourceModifyCmd cmd) {
		sourcesServiceI.modifySource(cmd);
	}

	@DeleteMapping("/v1/sources")
	@PreAuthorize("hasAuthority('write') and hasAuthority('sys:source:remove')")
	...
	@PreAuthorize("hasAuthority('write') and hasAuthority('sys:source:import')")
	...
	@PreAuthorize("hasAuthority('write') and hasAuthority('sys:source:export')")
	...
	@Cache(name = NameConstants.SOURCES, key = "#id")
	@PreAuthorize("hasAuthority('read') and hasAuthority('sys:source:detail')")
