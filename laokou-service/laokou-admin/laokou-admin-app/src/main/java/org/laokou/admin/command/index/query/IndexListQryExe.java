...
public class IndexListQryExe {
    ...
    public Result<Datas<IndexCO>> execute(IndexListQry qry) {
        Map<String, String> indexNames = elasticsearchTemplate.getIndexNames(new String[]{RESOURCE_INDEX, TRACE_INDEX});
        ...
        indexNames.forEach((indexName, indexAlias) -> list.add(new IndexCO(indexName, indexAlias)));
        Datas<IndexCO> datas = new Datas<>(list.size(), list.stream().skip((long) (pageNum - 1) * pageSize).limit(pageSize).toList());
