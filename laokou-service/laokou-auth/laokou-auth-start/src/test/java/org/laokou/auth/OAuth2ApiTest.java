...
public class OAuth2ApiTest {
    ...
    private String getUsernamePasswordAuthApi(long uuid, String captcha, String username, String password) {
        ...
        MvcResult mvcResult = mockMvc.perform(post(apiUrl).contentType(MediaType.MULTIPART_FORM_DATA_VALUE)).andExpect(status().isOk()).andReturn();
    ...
    private String getSecretsApi() {
        ...
        MvcResult mvcResult = mockMvc.perform(get(apiUrl).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andReturn();
