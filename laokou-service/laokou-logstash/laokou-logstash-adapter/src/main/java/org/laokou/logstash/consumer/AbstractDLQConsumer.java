...
import org.laokou.common.i18n.common.constant.TraceConstant;
...
public abstract class AbstractDLQConsumer implements RocketMQListener<MessageExt> {

	private final RocketMqTemplate rocketMqTemplate;

	@Override
	public void onMessage(MessageExt messageExt) {
		String traceId = messageExt.getProperty(TraceConstant.TRACE_ID);
		String spanId = messageExt.getProperty(TraceConstant.SPAN_ID);
		...
		try {
			if (StringUtil.isNotEmpty(tag)) {
				...
				rocketMqTemplate.sendAsyncMessage(topic, StringConstant.EMPTY, payload, traceId, spanId);
			}
		}
		finally {
			MDCUtil.clear();
