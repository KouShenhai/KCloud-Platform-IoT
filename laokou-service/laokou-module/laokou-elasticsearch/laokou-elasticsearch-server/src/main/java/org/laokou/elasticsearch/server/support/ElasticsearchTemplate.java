...
//                log.error("索引[{}],主键[{}]更新操作失败，状态为:[{}],错误信息:{}",indexName,item.id(),item.status(),item.error().reason());
//            }
//            return false;
//        }
//        // 记录索引新增与修改数量
...
//        for (BulkResponseItem item : bulk.items()) {
//            if (OperationType.Create.equals(item.operationType())) {
//                createdCount++;
//            } else if (OperationType.Update.equals(item.operationType())){
//                updatedCount++;
//            }
//        }
...
//            if (bulk.errors()) {
//                for (BulkResponseItem item : bulk.items()) {
//                    log.error("索引【{}】,主键[{}]修改操作失败，状态为【{}】,错误信息：{}",indexName,item.status(),item.error().reason());
//                }
//                return false;
//            }
//            // 记录索引新增与修改数量
...
//        jsonList.forEach(obj ->{
//            final Map<String, Object> map = (Map<String, Object>) obj;
//            builder.operations(item -> item.index(idx ->
//                    idx.index(indexName)
//
//                            .id(map.get(PRIMARY_KEY_NAME).toString())
//                            .document(obj)));
//        });
...
//        };
//        // 异步执行
//        elasticsearchClient.index(createIndexRequest, listener);
//    }
//
...
//        //封装es索引的mapping
//        GetMappingRequest.Builder builder1 = new GetMappingRequest.Builder();
//        TypeMapping mapping = packEsMapping(fieldMappingList, null);
...
//        builder.aliases(indexAlias,new Alias.Builder().isWriteIndex(true).build());
...
//     * @return
//     */
//    private TypeMapping packEsMapping(List<FieldMapping> fieldMappingList,TypeMapping mapping) {
//
//        if (mapping == null) {
//            // 如果对象是空，首次进入，设置开始结点
//            mapping = new TypeMapping.Builder()XContentFactory.jsonBuilder().startObject()
//                    .field("dynamic",true)
