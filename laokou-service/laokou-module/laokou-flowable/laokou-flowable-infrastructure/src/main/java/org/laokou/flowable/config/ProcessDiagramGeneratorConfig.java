...
public class ProcessDiagramGeneratorConfig extends DefaultProcessDiagramGenerator {
	...
	protected DefaultProcessDiagramCanvas generateProcessDiagram(BpmnModel bpmnModel, String imageType,
		...
		label75: while (true) {
			...
			while (ObjectUtils.isNull(subProcesses2));
			...
			while (true) {
				...
				do {
					...
					while (ObjectUtils.isNotNull(graphicInfo) && ObjectUtils.isNotNull(graphicInfo.getExpanded())
	...
	private static ProcessNum getProcessNum2(double minX, double maxX, double minY, double maxY, BpmnModel bpmnModel,
		...
		while (var27.hasNext()) {
			...
			if (ObjectUtils.isNotNull(var31)) {
			...
			if (ObjectUtils.isNotNull(var33)) {
	...
	protected void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel,
		...
		for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
			...
			boolean isDefault = ObjectUtils.isNotNull(defaultFlow) && defaultFlow.equalsIgnoreCase(sequenceFlow.getId());
			boolean drawConditionalIndicator = ObjectUtils.isNotNull(sequenceFlow.getConditionExpression())
			...
			if (ObjectUtils.isNotNull(graphicInfoList) && !graphicInfoList.isEmpty()) {
				...
				if (ObjectUtils.isNotNull(labelGraphicInfo)) {
	...
	private void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode,
		...
		if (ObjectUtils.isNotNull(drawInstruction)) {
			...
			if (flowNode instanceof Activity activity) {
				...
				if (ObjectUtils.isNotNull(multiInstanceLoopCharacteristics)) {
			...
			if (flowNode instanceof SubProcess) {
				collapsed = ObjectUtils.isNotNull(graphicInfo.getExpanded()) && !graphicInfo.getExpanded();
			}
