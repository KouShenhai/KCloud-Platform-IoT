...
public final class NacosShardingSphereURL {
	 ...
	 */
	public static NacosShardingSphereURL parse(final String url) {
		String sourceType = parseSourceType(url);
		return new NacosShardingSphereURL(sourceType, parseConfigurationSubject(url.substring(sourceType.length())),
				parseProperties(url));
	}
