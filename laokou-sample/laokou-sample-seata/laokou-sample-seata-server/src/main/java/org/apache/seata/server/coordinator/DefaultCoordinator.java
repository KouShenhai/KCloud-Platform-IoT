...
public class DefaultCoordinator extends AbstractTCInboundHandler implements TransactionMessageHandler, Disposable {
	...
	private final ScheduledThreadPoolExecutor syncProcessing = new ScheduledThreadPoolExecutor(1,
			new NamedThreadFactory(SYNC_PROCESSING, 1));

	private final GlobalStatus[] retryRollbackingStatuses = new GlobalStatus[] { GlobalStatus.TimeoutRollbacking,
	...
	protected DefaultCoordinator(RemotingServer remotingServer) {
		...
		boolean enableBranchAsyncRemove = CONFIG.getBoolean(ConfigurationKeys.ENABLE_BRANCH_ASYNC_REMOVE,
		...
		if (enableBranchAsyncRemove && StoreConfig.getSessionMode() != StoreConfig.SessionMode.FILE) {
			branchRemoveExecutor = new ThreadPoolExecutor(BRANCH_ASYNC_POOL_SIZE, BRANCH_ASYNC_POOL_SIZE,
					...
					new ArrayBlockingQueue<>(CONFIG.getInt(ConfigurationKeys.SESSION_BRANCH_ASYNC_QUEUE_SIZE,
	...
	protected void handleAsyncCommitting() {
		...
		Collection<GlobalSession> asyncCommittingSessions = SessionHolder.getRootSessionManager()
			.findGlobalSessions(sessionCondition);
	...
	protected void handleRollbackingByScheduled() {
		...
		sessionCondition.setLazyLoadBranch(true);
		List<GlobalSession> rollbackingSessions = SessionHolder.getRootSessionManager()
			.findGlobalSessions(sessionCondition);
		if (CollectionUtils.isEmpty(rollbackingSessions)) {
		...
		SessionHelper.forEach(needDoRollbackingSessions, rollbackingSession -> {
			try {
				...
				LOGGER.error("Failed to handle rollbacking [{}] {} {}", rollbackingSession.getXid(), ex.getCode(),
						ex.getMessage());
			}
		});
	...
	protected void handleCommittingByScheduled() {
		...
		sessionCondition.setLazyLoadBranch(true);
		List<GlobalSession> committingSessions = SessionHolder.getRootSessionManager()
			.findGlobalSessions(sessionCondition);
		if (CollectionUtils.isEmpty(committingSessions)) {
		...
		SessionHelper.forEach(needDoCommittingSessions, committingSession -> {
			try {
				...
				LOGGER.error("Failed to handle committing [{}] {} {}", committingSession.getXid(), ex.getCode(),
						ex.getMessage());
			}
		});
