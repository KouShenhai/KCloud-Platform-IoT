...
public class SessionHolder {
	...
	public static void init(SessionMode sessionMode) {
		...
		if (SessionMode.DB.equals(sessionMode)) {
			...
			ROOT_VGROUP_MAPPING_MANAGER = EnhancedServiceLoader.load(VGroupMappingStoreManager.class,
			...
			if (SessionMode.RAFT.equals(sessionMode)) {
				...
				String sessionStorePath = CONFIG.getConfig(ConfigurationKeys.STORE_FILE_DIR,
						DEFAULT_SESSION_STORE_FILE_DIR) + separator + System.getProperty(SERVER_SERVICE_PORT_CAMEL);
				if (StringUtils.isBlank(sessionStorePath) || StringUtils.isBlank(vGroupMappingStorePath)) {
					throw new StoreException("the {store.file.dir} is empty.");
				}
				ROOT_VGROUP_MAPPING_MANAGER = EnhancedServiceLoader.load(VGroupMappingStoreManager.class,
						SessionMode.FILE.getName(), new Object[] { vGroupMappingStorePath });
				...
				switch (globalStatus) {
					...
					case CommitRetrying:
						if (Objects.equals(SessionMode.RAFT, storeMode)) {
							// When a state change occurs, re-electing the leader may
							// result in the lock not being unlocked yet
							// so a COMMIT unlock operation needs to be performed at the
							// time of re-election
							try {
								globalSession.clean();
							}
