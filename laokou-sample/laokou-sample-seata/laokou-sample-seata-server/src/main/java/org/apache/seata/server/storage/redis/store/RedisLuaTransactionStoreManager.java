...
public class RedisLuaTransactionStoreManager extends RedisTransactionStoreManager {
	...
	private static final String UPDATE_BRANCH_TRANSACTION_DO_LUA_FILE_NAME = LUA_PREFIX
			+ "updateBranchTransactionDO.lua";

	private static final String UPDATE_GLOBAL_TRANSACTION_DO_LUA_FILE_NAME = LUA_PREFIX
			+ "updateGlobalTransactionDO.lua";

	private static final String ROLLBACK_GLOBAL_TRANSACTION_DO_LUA_FILE_NAME = LUA_PREFIX
			+ "rollbackGlobalTransactionDO.lua";

	/**
	 * key filename value LOCK_SHA_SCRIPT_ID
	 */
	...
	protected boolean updateBranchTransactionDO(BranchTransactionDO branchTransactionDO) {
		...
		try (Jedis jedis = JedisPooledFactory.getJedisInstance()) {
			...
			String result = (String) LuaParser.jedisEvalSha(jedis, luaSHA, UPDATE_BRANCH_TRANSACTION_DO_LUA_FILE_NAME,
					keys, args);
	...
	protected boolean updateGlobalTransactionDO(GlobalTransactionDO globalTransactionDO) {
		...
		try (Jedis jedis = JedisPooledFactory.getJedisInstance()) {
			...
			String result = (String) LuaParser.jedisEvalSha(jedis, luaSHA, UPDATE_GLOBAL_TRANSACTION_DO_LUA_FILE_NAME,
					keys, args);
			LuaParser.LuaResult luaResult = LuaParser.getObjectFromJson(result, LuaParser.LuaResult.class);
			// fail
			if (!luaResult.getSuccess()) {
