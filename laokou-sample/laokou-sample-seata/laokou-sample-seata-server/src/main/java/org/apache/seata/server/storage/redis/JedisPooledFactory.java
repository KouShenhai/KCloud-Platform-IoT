				...
				if (jedisPool == null) {
					...
					if (jedisPools != null && jedisPools.length > 0) {
						...
						if (mode.equals(ConfigurationKeys.REDIS_SENTINEL_MODE)) {
							...
							String sentinelPassword = CONFIGURATION
								.getConfig(ConfigurationKeys.STORE_REDIS_SENTINEL_PASSWORD);
							if (StringUtils.isBlank(sentinelPassword)) {
								sentinelPassword = null;
							}
							tempJedisPool = new JedisSentinelPool(masterName, sentinels, poolConfig, 60000, 60000,
									password, CONFIGURATION.getInt(ConfigurationKeys.STORE_REDIS_DATABASE, DATABASE),
