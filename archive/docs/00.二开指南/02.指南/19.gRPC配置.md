...
```yaml
# laokou-distributed-id-segment 服务端配置
spring:
  grpc:
    server:
      servlet:
...
spring:
  grpc:
    client:
      channels:
        laokou-distributed-id-segment:                        # 通道名称（对应目标服务名）
          address: discovery://laokou-distributed-id-segment   # 通过 Nacos 服务发现
...
// 3. 定义消息体（Message）
messageHandler GenerateIdRequest {
    // 空消息体
}

messageHandler GenerateBatchIdRequest {
    int32 num = 1;          // 字段编号从 1 开始
}

messageHandler GenerateIdResponse {
    string code = 1;        // 状态码
    string msg = 2;         // 状态消息
    int64 data = 3;         // 生成的 ID
}

messageHandler GenerateBatchIdsResponse {
    string code = 1;
    string msg = 2;
...
| 选项                     | 值      | 说明                                       |
|------------------------|--------|------------------------------------------|
| `java_multiple_files`  | `true` | 每个 `messageHandler` / `service` 生成独立 `.java` 文件 |
...
```
laokou-distributed-id-segment-proto/
├── pom.xml
├── src/
│   └── main/
│       └── proto/
...
```bash
# 方式一：完整编译（推荐）
cd laokou-service/laokou-distributed-id-segment/laokou-distributed-id-segment-proto
mvn compile
...
messageHandler XxxRequest {
    string name = 1;
}

messageHandler XxxResponse {
    string code = 1;
    string msg = 2;
    string data = 3;
}
