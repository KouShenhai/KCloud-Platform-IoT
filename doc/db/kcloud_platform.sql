...
create table if not exists public.boot_sys_dict
(
	id          bigint generated by default as identity
		primary key,
	creator     bigint        default 0                      not null,
...
comment on column public.boot_sys_dict.dept_id is '部门ID';

comment on column public.boot_sys_dict.dept_path is '部门PATH';
...
alter table public.boot_sys_dict
	owner to root;

create unique index if not exists type_tenant_id_idx
	on public.boot_sys_dict (type, tenant_id);

comment on index public.type_tenant_id_idx is '类型_租户ID_唯一索引';

INSERT INTO public.boot_sys_dict (id, creator, editor, create_time, update_time, del_flag, version, dept_id,
