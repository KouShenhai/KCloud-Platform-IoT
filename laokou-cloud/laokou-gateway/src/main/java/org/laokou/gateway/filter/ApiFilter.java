...
public class ApiFilter implements WebFilter {
    ...
    private Mono<Void> validate(ServerWebExchange exchange,ServerHttpRequest request, Auth auth, WebFilterChain chain) {
        try {
            ...
            if (StringUtil.isEmpty(username)) {
                return ReactiveResponseUtil.response(exchange, Result.fail(ValidatorUtil.getMessage(OAUTH2_USERNAME_REQUIRE)));
            }
            if (StringUtil.isEmpty(password)) {
                return ReactiveResponseUtil.response(exchange, Result.fail(ValidatorUtil.getMessage(OAUTH2_PASSWORD_REQUIRE)));
            }
            try {
